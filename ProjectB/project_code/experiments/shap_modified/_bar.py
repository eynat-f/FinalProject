...
from code import Cohorts, Explanation
...
def bar(
    ...
    for i in feature_inds:
        if features is not None and show_data:
            yticklabels.append(format_value(features[i], "%+0.5f") + " = " + feature_names[i])
        else:
        ...
        for j in range(len(y_pos)):
            ...
            if values[i, ind] < 0:
                ax.text(
                    ...
                    y_pos[j] + ypos_offset,
                    format_value(values[i, ind], "%+0.5f"),
    ...
    if partition_tree is not None:
        ...
        ax.text(
            ...
            (ymax - ymin) / 2,
            "Clustering cutoff = " + format_value(clustering_cutoff, "%+0.5f"),
            horizontalalignment="left",
            verticalalignment="center",
            color="#999999",
            fontsize=12,
