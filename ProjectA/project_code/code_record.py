...
def fly_cf():
    ...
    with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        # if we get loco deck- can switch to: logconf.add_variable('locSrv.x', 'float'), logconf.add_variable(
        # 'locSrv.y', 'float')
...
def record_cf():
    ...
    inp = alsaaudio.PCM(alsaaudio.PCM_CAPTURE, alsaaudio.PCM_NONBLOCK, channels=1, rate=FS,
                        format=alsaaudio.PCM_FORMAT_S16_LE, periodsize=1024, device='default')
